...
async function gscQuery(siteUrl, body) {
  ...
  const r = await fetch(
    `https://www.googleapis.com/webmasters/v3/sites/${encodeURIComponent(siteUrl)}/searchAnalytics/query`,
    {
      ...
      // dataState:"all" includes the most recent, not-yet-finalized days so totals
      // match the Search Console web UI (which shows fresh data) rather than being
      // slightly lower (the API defaults to finalized data only).

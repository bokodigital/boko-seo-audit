...
// Google-standard length thresholds (2026 guidance).
// Title: too short <30-50, ideal 50-60 (~569px), too long >60 (gets cut off).
// Description: too short <70-120, ideal 120-158 (~400-920px), too long >160 (truncated with an ellipsis).
// Alt text: keep under 125 chars so screen readers don't cut it off.
// og:title: ideal 40-60 chars, hard max 88 before social feeds (Facebook/LinkedIn) cut it off.
const T_MIN = 50, T_MAX = 60;
const D_MIN = 120, D_MAX = 158;
const ALT_MAX = 125;
const OG_TITLE_MIN = 40, OG_TITLE_MAX = 60, OG_TITLE_HARD_MAX = 88;
...
function analyzeHtml(html, finalUrl, headers, chain) {
  ...
  // title (Google ideal: 50-60 chars, ~569px)
  const title = firstMatch(/<title[^>]*>([\s\S]*?)<\/title>/i, html);
  const tlen = title ? title.length : 0;
  checks.push(ck("title", "Title tag", !title ? "fail" : (tlen < T_MIN || tlen > T_MAX) ? "warn" : "pass",
    title ? `"${title}" (${tlen} chars)` : "Missing <title>.",
    !title ? "Add a unique, descriptive <title>." : (tlen > T_MAX ? `Shorten the title to ${T_MIN}-${T_MAX} characters so it isn't cut off in search results.` : tlen < T_MIN ? `Lengthen the title to ${T_MIN}-${T_MAX} characters to make better use of available keyword/branding space.` : ""), 3));

  // meta description (Google ideal: 120-158 chars, ~400-920px)
  const desc = metaContent(html, "description");
  const dlen = desc ? desc.length : 0;
  checks.push(ck("description", "Meta description", desc == null ? "fail" : (dlen < D_MIN || dlen > D_MAX) ? "warn" : "pass",
    desc == null ? "Missing meta description." : `${dlen} chars`,
    desc == null ? `Add a compelling meta description (~${D_MIN}-${D_MAX} chars).` : (dlen > D_MAX ? `Trim the meta description to ${D_MIN}-${D_MAX} characters — it will be truncated with an ellipsis beyond ~160.` : dlen < D_MIN ? `Expand the meta description to ${D_MIN}-${D_MAX} characters to make full use of the pitch.` : ""), 2));
  ...
  // images missing alt, or alt text over Google's recommended length
  const imgTagsAll = html.match(/<img\b[^>]*>/gi) || [];
  const imgs = imgTagsAll.length;
  const imgsNoAlt = imgTagsAll.filter(t => !/\balt=/i.test(t) || /\balt=["']\s*["']/i.test(t)).length;
  const imgsAltTooLong = imgTagsAll.filter(t => {
    const m = t.match(/\balt=["']([^"']*)["']/i);
    return m && m[1].length > ALT_MAX;
  }).length;
  const altIssues = imgsNoAlt + imgsAltTooLong;
  checks.push(ck("img-alt", "Image alt text", altIssues === 0 ? "pass" : imgsNoAlt > imgs / 2 ? "fail" : "warn",
    `${imgsNoAlt} of ${imgs} images missing alt` + (imgsAltTooLong ? `; ${imgsAltTooLong} with alt text over ${ALT_MAX} characters.` : "."),
    altIssues ? `Add descriptive alt text under ${ALT_MAX} characters to every image for accessibility & image SEO.` : "", 1));
  ...
  // open graph (og:title ideal 40-60 chars, hard cap ~88 before social feeds truncate it)
  const ogTitle = metaContent(html, "og:title");
  const ogImage = metaContent(html, "og:image");
  const ogTitleLen = ogTitle ? ogTitle.length : 0;
  const ogTitleTooLong = !!ogTitle && ogTitleLen > OG_TITLE_HARD_MAX;
  const ogTagsPresent = !!(ogTitle && ogImage);
  checks.push(ck("opengraph", "Open Graph tags", ogTagsPresent && !ogTitleTooLong ? "pass" : "warn",
    !ogTagsPresent
      ? "Missing og:title and/or og:image."
      : ogTitleTooLong
        ? `og:title is ${ogTitleLen} characters — over the ~${OG_TITLE_HARD_MAX} char limit, so it may be cut off on social feeds.`
        : "og:title and og:image present.",
    !ogTagsPresent
      ? "Add Open Graph tags (og:title, og:description, og:image) for better social sharing."
      : ogTitleTooLong
        ? `Shorten og:title to ${OG_TITLE_MIN}-${OG_TITLE_MAX} characters (max ${OG_TITLE_HARD_MAX}) so it isn't cut off on Facebook/LinkedIn.`
        : "", 1));
...
function pageLevel(p){
  ...
  if (p.titleFlag !== "ok" || p.descFlag !== "ok" || p.imgsNoAlt > 0 || (p.imgsAltTooLong || 0) > 0 || !p.ogOk || p.ogTitleTooLong) return "warn";
  return "ok";
}
...
function analyzePageMeta(html, url, status = 200){
  ...
  const imgs = imgTags.length;
  const noAltTags = imgTags.filter(t => !/\balt=/i.test(t) || /\balt=["']\s*["']/i.test(t));
  const imgsNoAlt = noAltTags.length;
  const imgsNoAltList = noAltTags.map(t => {
    const m = t.match(/\bsrc=["']([^"']+)["']/i);
    let s = m ? m[1] : "";
    try { s = new URL(s, url).toString(); } catch (e) {}
    return s;
  }).filter(Boolean);
  // alt text present but over Google's recommended 125-char limit
  const altTooLongTags = imgTags.filter(t => {
    const m = t.match(/\balt=["']([^"']*)["']/i);
    return m && m[1].length > ALT_MAX;
  });
  const imgsAltTooLong = altTooLongTags.length;
  const imgsAltTooLongList = altTooLongTags.map(t => {
    const m = t.match(/\bsrc=["']([^"']+)["']/i);
    let s = m ? m[1] : "";
    try { s = new URL(s, url).toString(); } catch (e) {}
    return s;
  ...
  const h1Text = (firstMatch(/<h1[^>]*>([\s\S]*?)<\/h1>/i, html) || "").replace(/<[^>]+>/g, " ").replace(/\s+/g, " ").trim();
  ...
  const p = {
    ...
    imgs, imgsNoAlt, imgsNoAltList, imgsAltTooLong, imgsAltTooLongList,
    ogOk: !!(ogTitle && ogImage), ogMissing, ogTitleLen, ogTitleTooLong,
    h1, h1Text, noindex,
  };
  p.level = pageLevel(p);
  return p;
...
async function crawlPages(finalUrl, seedHtml, discoveredUrls){
  ...
  const summary = {
    ...
    altIssues: pages.filter(p => p.imgsNoAlt > 0 || (p.imgsAltTooLong || 0) > 0).length,
    ogMissing: pages.filter(p => !p.ogOk).length,
    ogTitleTooLong: pages.filter(p => p.ogTitleTooLong).length,
  };
...
async function discoverPages(origin, cap = 60) {
  const host = (() => { try { return new URL(origin).host; } catch (e) { return ""; } })();
  // Seed from the site's OWN sitemap index (robots.txt + common Yoast/WordPress
  // locations) AND any sitemaps submitted to Search Console. We MERGE both because
  // a site often submits only a partial sitemap to GSC (e.g. just page-sitemap.xml)
  // while its sitemap_index.xml lists posts, products, categories, CPTs, etc.
  const seeds = new Set();
  try { const smUrl = await findSitemapUrl(origin); if (smUrl) seeds.add(smUrl); } catch (e) {}
  seeds.add(origin + "/sitemap_index.xml");
  seeds.add(origin + "/sitemap.xml");
  let viaGsc = false;
  if (googleConfigured()) {
    for (const prop of [origin + "/", "sc-domain:" + host]) {
      try {
        const paths = await listSitemaps(prop);
        if (paths && paths.length) { paths.forEach((p) => seeds.add(p)); viaGsc = true; break; }
      } catch (e) { /* property not in this account / no access — ignore */ }
    }
  }

  const { urls, breakdown } = await urlsFromSitemaps([...seeds], cap);
  return {
    source: urls.length ? "sitemap" : "links",
    viaGsc,
    sitemapUrl: (breakdown[0] && breakdown[0].sitemap) || [...seeds][0] || null,
    total: urls.length,
    urls,
    breakdown,
  };
...
export async function discoverAllUrls(input) {
  ...
  return { origin, finalUrl: main.finalUrl, source: d.source, viaGsc: d.viaGsc, total: urls.length, urls, breakdown: d.breakdown || [] };
}

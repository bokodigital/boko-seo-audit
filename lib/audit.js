...
// Title: ideal 30-60 chars — only flag outside that range.
// Description: ideal 120-158 (~400-920px), too long >158 (truncated with an ellipsis).
// Alt text: only flagged when missing/empty — any alt value passes.
// og:title: only flagged when missing — any og:title value passes.
...
function analyzeHtml(html, finalUrl, headers, chain) {
  ...
  // images missing alt (any alt value counts as a pass — length is not flagged)
  ...
  checks.push(ck("img-alt", "Image alt text", imgsNoAlt === 0 ? "pass" : imgsNoAlt > imgs / 2 ? "fail" : "warn",
    ...
    imgsNoAlt ? "Add descriptive alt text to every image for accessibility & image SEO." : "", 1));
  ...
  // open graph (any og:title value passes — length is not flagged)
  const ogTitle = metaContent(html, "og:title");
  const ogImage = metaContent(html, "og:image");
  const ogTagsPresent = !!(ogTitle && ogImage);
  checks.push(ck("opengraph", "Open Graph tags", ogTagsPresent ? "pass" : "warn",
    ogTagsPresent ? "og:title and og:image present." : "Missing og:title and/or og:image.",
    ogTagsPresent ? "" : "Add Open Graph tags (og:title, og:description, og:image) for better social sharing.", 1));
...
function analyzePageMeta(html, url, status = 200){
  ...
  const p = {
    ...
    imgs, imgsNoAlt, imgsNoAltList,
    ogOk: !!(ogTitle && ogImage), ogMissing, ogTitleLen,
    h1, h1Text, noindex,
  };
  p.level = pageLevel(p);
  return p;
